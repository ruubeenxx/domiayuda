...
export default function Inicio() {
  const { state, ganado, gastadoHoy, propinas, domisHoy, balance, diasRestantes, porDia, domisNecesarios, domisBase } = useApp()
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <div style={{ fontSize: 26, fontWeight: 800 }}>{fmt(ganado)} <span style={{ fontSize: 13, color: 'var(--text3)', fontWeight: 500 }}>ganados</span></div>
      ...
      <div className="stat-grid">
        <div className="stat-card">
          ...
          <div className="stat-val sv-green">{fmt(20000 + balance)}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Balance día</div>
          <div className="stat-val" style={{ color: balance >= 0 ? 'var(--green)' : 'var(--red)' }}>{fmt(balance)}</div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text3)', fontWeight: 600, marginBottom: 5 }}>
          <span>{domisHoy} de {domisNecesarios} domis</span>
        ...
        {state.domisPendientes > 0 && (
          <div style={{ fontSize: 11, color: 'var(--amber)', marginTop: 5, fontWeight: 600 }}>
            +{state.domisPendientes} acumulados de ayer
          </div>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div className="list-item">
          <div className="li-left"><span className="li-icon">⬆️</span>Ingresos</div>
          <span style={{ color: 'var(--green)', fontWeight: 700, fontSize: 13 }}>{fmt(ganado)}</span>
        </div>
        <div className="list-item">
          <div className="li-left"><span className="li-icon">⬇️</span>Gastos</div>
